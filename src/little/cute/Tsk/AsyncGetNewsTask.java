...
public class AsyncGetNewsTask extends AsyncTask<String, Integer, Long> implements OnCancelListener{
	  ...
	  /// 呼び出し順序:1
	  /// </summary>
	  public AsyncGetNewsTask(LiplisNews lpsnews,Context context, List<NameValuePair> nameValuePair){
	  ...
	  /// <summary>
	  /// doInBackground
	  /// 2013/03/01 ver3.3.0 一括取得方式に移行するため廃止
	  /// </summary>
	  protected Long doInBackgroundOld(String... params) {
		  ...
		  for(int i=0; i< 50; i++){
		    ...
		    Log.d(TAG, i + "件目ダウンロード ");

		    try
		    {
		    	//ニュース取得
		    	MsgShortNews n = LiplisApi.getShortNews(nameValuePair, 0);

			    //取得件数が0以上なら採用
			    if(n.nameList.size() > 0)
			    {
			    	lpsnews.newsQueue.add(n);
			    }
		    }
		    catch(Exception e)
		    {
